...
MANIFEST = BASE_DIR / "schema" / "layer_b" / "manifest.json"
...
def apply_layer_b_migrations(config: AppConfig) -> list[str]:
    bootstrap_core_databases(config)
    driver = connect_with_retry(config)
    applied: list[str] = []
    try:
        db = driver.databases.get(config.layer_b_db)
        ...
        for migration in load_manifest():
            ...
            with driver.transaction(config.layer_b_db, TransactionType.SCHEMA) as tx:
